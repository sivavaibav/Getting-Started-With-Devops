...
cd ..
docker pull sivavaibav/sample:latest . // replace with your docker uid/repo:image_tag
cd docker
```
...
```bash
docker build -t sivavaibav/sample/latest // replace with your docker uid/repo:image_tag
docker ps -a
```
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: my-app
        image: sivavaibav/docker:latest
